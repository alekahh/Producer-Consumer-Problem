        ...
        case 0:
            ...
            do {
                ...
                if (strcmp(buffer_start, "THIS_IS_THE_END") == 0) {
                    ...
                    long int multi_threads = multi_threads_run();
                    total_word_count += multi_threads;
                    if (shared_mem[0] == '1') {
                        total_word_count--;
                        if (shared_mem[1] == '1') total_word_count++;
                    }
                }
            ...
            saveResult("p3_result.txt", total_word_count);
...
//this functoin is called after threads all received their correspoing chunks, the function calculates the number of words
void *thread_func(void *arg) {

    thread_data *threadData = (thread_data *) arg;

    int size = threadData->end - threadData->start + 1;
    char *substring = (char *)malloc(size + 1);
    substring = strncpy (substring, shared_mem + threadData->start, size);
    substring[size] = '\0';

    threadData->word_count = wordCount(substring);
    
    pthread_exit(NULL);
}
  //threads are intilized in this function
long int multi_threads_run()
{       

    
    int size = strlen(shared_mem);
    thread_data thread_data[4];
    pthread_t threads[4];

    //this loops separates the string in the shared memory into roughly four equal parts,
    for (int i = 0; i < 4; i++) {

        if (i > 0) thread_data[i].start = (size / 4) * i + 1;
        else thread_data[i].start = 0;
        if (i < 3) thread_data[i].end = (size / 4) * (i + 1);
        else thread_data[i].end = size;
